        ...
        while cap.isOpened():
            ...
            try:
                ...
                if len(results) > 0 and hasattr(results[0], 'keypoints') and len(results[0].keypoints) > 0:
                    ...
                    # Handle different YOLO versions - some return 51 dims (17*3), some return 17 dims
                    if keypoints.shape[0] == 0:
                        print("No person detected in frame")
                        continue
                    
                    # If we have 51 dimensions, reshape to (17, 3)
                    if len(keypoints.shape) == 3 and keypoints.shape[2] == 51:
                        keypoints = keypoints.reshape(keypoints.shape[0], 17, 3)
                    elif len(keypoints.shape) == 2 and keypoints.shape[1] == 51:
                        keypoints = keypoints.reshape(keypoints.shape[0], 17, 3)
                    
                    # Verify keypoints shape is valid
                    if keypoints.shape[0] == 0 or keypoints.shape[1] != 17 or keypoints.shape[2] != 3:
                        print(f"Invalid keypoints shape: {keypoints.shape}")
                        continue
...
def main():
    ...
    references_path = r"angles_final.pkl"
    save_keypoints = True
