...
class FeedbackManager:
    ...
    def generate_feedback(self, joint_name, current_angle, target_angle, tolerance):
        ...
        if severity == "major":
            feedback = "!!!WARNING " + feedback + f" (off by {abs(diff):.0f} value)"
        ...
        if severity in ['major', 'extreme']:
            if characteristics:
                ...
                if current_angle < min_val:
                    enhanced_feedback += f" (too low by {min_val - current_angle:.0f} value)"
                elif current_angle > max_val:
                    enhanced_feedback += f" (too high by {current_angle - max_val:.0f} value)"
                else:
                    enhanced_feedback += f" (off by {abs_diff:.0f} value)"
            else:
                enhanced_feedback += f" (off by {abs_diff:.0f} value)"
...
class GuidedZumbaAnalyzer:
    ...
    def __init__(
        ...
        reference_file: str = r"src/improved_automatic_references.json",
        ...
        try:
            ...
            self.voice.setProperty("rate", 150)  # Slightly faster for real-time
            self.voice.setProperty("volume", 1.0)  # Max volume
            # Try to set a female voice which is often clearer for instructions
            ...
            print("💡 Installing SAPI5 voices may help: 'pip install pyttsx3 && python -m pyttsx3.drivers.sapi5'")
        ...
        self.voice_feedback_count = 0  # Track voice feedback attempts
        
        # Feedback queue for pacing
        self.feedback_queue = deque(maxlen=3)
        self.current_issues = {}
        self.issue_persistence = {}
        self.feedback_message_list = []  # Initialize feedback message list
    ...
    def compare(self, user_angles):
        ...
        # Store detailed feedback for voice feedback system
    ...
    def _add_feedback_to_list(self, message):
        """Add feedback message to the display list"""
        self.feedback_message_list.append(message)
        ...
        if priority_issues and now - self.last_voice_ts >= self.min_feedback_gap:
            ...
            if self.voice_enabled:
                try:
                    ...
                    self.voice_feedback_count += 1
                    print(f"🗣️ Voice feedback #{self.voice_feedback_count}: {msg}")
                except Exception as e:
                    print(f"⚠️ Voice feedback failed: {e}")
                    # Don't disable voice on single failure, might be temporary
    ...
    def _draw_enhanced_overlay(self, frame, bad_parts, keypoints=None):
        ...
        # Show real-time accuracy with rolling window for better responsiveness
        if self.performance_metrics["total_frames"] > 0:
            # Use rolling window of last 20 frames for more responsive accuracy
            recent_frames = self.performance_metrics.get("recent_frames", [])
            
            # Add current frame result to recent frames
            if not bad_parts:
                recent_frames.append(1)  # Good frame
            else:
                recent_frames.append(0)  # Bad frame
            
            # Keep only last 20 frames
            if len(recent_frames) > 20:
                recent_frames.pop(0)
            
            self.performance_metrics["recent_frames"] = recent_frames
            
            # Calculate accuracy based on recent frames
            if len(recent_frames) > 0:
                accuracy = (sum(recent_frames) / len(recent_frames)) * 100
            else:
                accuracy = (self.performance_metrics["good_frames"] / 
                          self.performance_metrics["total_frames"]) * 100
            
            # Set color based on current form status
            if not bad_parts:
                acc_color = (0, 255, 0)  # Green for good form
            else:
                acc_color = (0, 165, 255)  # Orange for issues
    ...
    def guide_live(self, target_move, cam_id=0):
            ...
            if patterns:
                ...
                for pattern, joints in patterns.items():
                    if pattern == 'cyclic':
                        ...
                        print(f"      Static (hold steady): {', '.join(joints[:3])}")
                    elif pattern == 'variable':
                        print(f"      Variable: {', '.join(joints[:3])}")
        ...
        if not cap.isOpened():
            print(f"  Cannot open camera with ID {cam_id}")
            return
        else:
            ...
            print(f" Camera with ID {cam_id} opened successfully")
            ...
            # Create a named window with the normal flag
            ...
            frame_count = 0
            last_process_time = 0
            process_interval = 0.5  # Process every 0.5 seconds for ultra-smooth
        ...
        while True:
            ...
            res = self.pose_model(frm, verbose=False, conf=0.5, iou=0.45)
            bad = []  # Default value if no keypoints detected or condition not met
            kps = None  # Default value if no keypoints detected
            message = "Please show your full body"  # Default message

            if res and res[0].keypoints is not None and len(res[0].keypoints.data) > 0:
                kps = res[0].keypoints.data[0].cpu().numpy()
                # Debug: Show detection status every 2 seconds
                shoulder_conf = max(kps[5][2], kps[6][2])
                foot_conf = max(kps[15][2], kps[16][2])
                if frame_count % 360 == 0:  # Even less frequent prints for performance
                    print(f" Detection - Shoulder: {shoulder_conf:.2f}, Foot: {foot_conf:.2f}")
                
                # Lower confidence threshold from 0.5 to 0.3 for better detection
                if (kps[5][2] > 0.3 or kps[6][2] > 0.3) and (kps[15][2] > 0.3 or kps[16][2] > 0.3):
                    ang = self._extract_angles(kps)
                    bad = self.compare(ang)
                    
                    # Update performance metrics
                    self.performance_metrics["total_frames"] += 1
                    if not bad:  # Good form
                        self.performance_metrics["good_frames"] += 1
                    
                    # Provide feedback at controlled pace - every 0.5 seconds
                    current_time = cv2.getTickCount() / cv2.getTickFrequency()
                    if current_time - last_process_time >= process_interval:
                        self._maybe_feedback(bad)
                        last_process_time = current_time
                    message = None  # Clear message if condition is met

            # Draw the message if it exists
            if message:
                cv2.putText(frm, message, (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
...
def process_with_feedback(input_video: str, target_move: str, output_video: str | None = None, include_audio: bool = True):
    ...
    # Load reference data
    analyzer.load_references("src/improved_automatic_references.json")
    ...
    for fnum in range(tot):
        ...
        if res and res[0].keypoints is not None and len(res[0].keypoints.data) > 0:
            ...
            # Update performance metrics for every frame
            analyzer.performance_metrics["total_frames"] += 1
            
            if fnum % every == 0:
                last_bad = analyzer.compare(ang)
                if not last_bad:  # Good form
                    analyzer.performance_metrics["good_frames"] += 1
            ...
            # No person detected, just draw the frame without skeleton
            analyzer._draw_enhanced_overlay(frm, [], None)
    ...
    print(f"    ▸ Progress: 100% ({tot}/{tot} frames) ✅")
